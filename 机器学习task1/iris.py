...
#from sklearn.model_selection import train_test_split这一句就已经搞定了换份数据集的任务

input, x_test, label, y_test = train_test_split(dataset.data, dataset.target, test_size=0.2, random_state=1)
...
'''
softmax完成了将数值对应概率的任务,其具体函数式和作用原理在task0中已作答
'''
...
class NET(nn.Module):
    def __init__(self, n_feature, n_hidden1, n_hidden2, n_output):
        ...
        self.softmax = nn.Softmax(dim=1)        #沿着第一列（维）计算
#前向传播函数
    def forward(self, x):
...
#查到iris数据集是一个包含了 150 个样本和 4 个特征的数据集，因此可以input.size(1/2/3)或者令其直接等于4
n_feature ,n_hidden1, n_hidden2, n_output = input.size(1), 20, 20, 3
...
#这一句将会清除掉计算图中权重梯度里的数据，不然随循环持续将一直+++
